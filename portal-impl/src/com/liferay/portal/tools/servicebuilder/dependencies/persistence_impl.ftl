	...
	<#if entity.isHierarchicalTree()>
		...
		protected List<Long> getChildrenTree${pkColumn.methodName}s(${entity.name} parent${entity.name}) throws SystemException {
			...
			try {
				...
				SQLQuery q = session.createSQLQuery("SELECT ${pkColumn.methodName} FROM ${entity.name} WHERE (${scopeColumn.name} = ?) AND (left${pkColumn.methodName} BETWEEN ? AND ?)");
		...
		protected void updateChildrenTree(long delta, long ${scopeColumn.name}, List<Long> children${pkColumn.methodName}s){
			String sql = "UPDATE ${entity.name} SET left${pkColumn.methodName} = (left${pkColumn.methodName} + ?), right${pkColumn.methodName} = (right${pkColumn.methodName} + ?) WHERE (${scopeColumn.name} = ?) AND (${pkColumn.methodName} IN (" + buildSqlInClause(children${pkColumn.methodName}s) +"))";
		...
		protected void expandNoChildrenLeft${pkColumn.methodName}(long delta, long ${scopeColumn.name}, long left${pkColumn.methodName}, List<Long> children${pkColumn.methodName}s){
			String sql = "UPDATE ${entity.name} SET left${pkColumn.methodName} = (left${pkColumn.methodName} + ?) WHERE (${scopeColumn.name} = ?) AND (left${pkColumn.methodName} > ?) AND (${pkColumn.methodName} NOT IN (" + buildSqlInClause(children${pkColumn.methodName}s) +"))";
		...
		protected void expandNoChildrenRight${pkColumn.methodName}(long delta, long ${scopeColumn.name}, long right${pkColumn.methodName}, List<Long> children${pkColumn.methodName}s){
			String sql = "UPDATE ${entity.name} SET right${pkColumn.methodName} = (right${pkColumn.methodName} + ?) WHERE (${scopeColumn.name} = ?) AND (right${pkColumn.methodName} > ?) AND (${pkColumn.methodName} NOT IN (" + buildSqlInClause(children${pkColumn.methodName}s) +"))";
