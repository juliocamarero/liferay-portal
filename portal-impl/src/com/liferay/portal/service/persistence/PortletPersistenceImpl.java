...
public class PortletPersistenceImpl extends BasePersistenceImpl<Portlet>
	...
	public static final FinderPath FINDER_PATH_COUNT_BY_PORTLETID = new FinderPath(PortletModelImpl.ENTITY_CACHE_ENABLED,
			PortletModelImpl.FINDER_CACHE_ENABLED, Long.class,
	...
	public Portlet updateImpl(com.liferay.portal.model.Portlet portlet,
		...
		if (isNew || !PortletModelImpl.COLUMN_BITMASK_ENABLED) {
			...
			if ((portletModelImpl.getColumnBitmask() &
				...
				args = new Object[] { portletModelImpl.getPortletId() };
	...
	public List<Portlet> findByPortletId(String portletId, int start, int end,
		...
		if ((list != null) && !list.isEmpty()) {
			for (Portlet portlet : list) {
				if (!Validator.equals(portletId, portlet.getPortletId())) {
					list = null;
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public Portlet findByPortletId_First(String portletId,
		OrderByComparator orderByComparator)
		throws NoSuchPortletException, SystemException {
		List<Portlet> list = findByPortletId(portletId, 0, 1, orderByComparator);
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public Portlet findByPortletId_Last(String portletId,
		OrderByComparator orderByComparator)
		throws NoSuchPortletException, SystemException {
		int count = countByPortletId(portletId);

		List<Portlet> list = findByPortletId(portletId, count - 1, count,
				orderByComparator);
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public Portlet[] findByPortletId_PrevAndNext(long id, String portletId,
		OrderByComparator orderByComparator)
		throws NoSuchPortletException, SystemException {
		Portlet portlet = findByPrimaryKey(id);
		...
		try {
			...
			array[0] = getByPortletId_PrevAndNext(session, portlet, portletId,
					orderByComparator, true);

			array[1] = portlet;

			array[2] = getByPortletId_PrevAndNext(session, portlet, portletId,
					orderByComparator, false);
	...
	protected Portlet getByPortletId_PrevAndNext(Session session,
		Portlet portlet, String portletId, OrderByComparator orderByComparator,
		boolean previous) {
		StringBundler query = null;
	...
	public void removeByPortletId(String portletId) throws SystemException {
		for (Portlet portlet : findByPortletId(portletId)) {
			remove(portlet);
		}
	}
	...
	private static final String _FINDER_COLUMN_PORTLETID_PORTLETID_1 = "portlet.portletId IS NULL";
	private static final String _FINDER_COLUMN_PORTLETID_PORTLETID_2 = "portlet.portletId = ?";
	private static final String _FINDER_COLUMN_PORTLETID_PORTLETID_3 = "(portlet.portletId IS NULL OR portlet.portletId = ?)";
