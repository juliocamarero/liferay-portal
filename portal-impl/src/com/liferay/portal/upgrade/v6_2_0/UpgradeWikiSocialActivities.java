...
public class UpgradeWikiSocialActivities extends UpgradeProcess {
	...
	private void addUpdatedSocialActivity(long groupId, long companyId,
		long userId, Timestamp modifiedDate, long classNameId,
		long resourcePrimKey, double version) throws Exception {
	...
	private void updateWikiPageSocialActivities() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				addUpdatedSocialActivity(groupId, companyId, userId,
					modifiedDate, classNameId, resourcePrimKey, version);
			}
		}
		finally {
			DataAccess.cleanUp(con, ps, rs);
