...
public class ExportLayoutsAction extends PortletAction {
	...
	private void addAllChildren(Layout layout, List<Long> childLayoutIds)
		throws Exception {

		addAllChildren(layout.getGroupId(), layout.isPrivateLayout(),
			layout.getLayoutId(), childLayoutIds);
	}

	private void addAllChildren(
			long groupId, boolean privateLayout, long layoutId,
			List<Long> childLayoutIds)
		throws Exception {

		List<Layout> localChildren =
			LayoutLocalServiceUtil.getLayouts(groupId, privateLayout, layoutId);

		for (Layout child : localChildren) {
			childLayoutIds.add(child.getLayoutId());
			addAllChildren(child, childLayoutIds);
		}
	}
