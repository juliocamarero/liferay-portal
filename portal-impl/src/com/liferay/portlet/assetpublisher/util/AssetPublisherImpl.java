...
public class AssetPublisherImpl implements AssetPublisher {
	...
	public List<AssetEntry> getAssetEntries(
		...
		if ((entries.size() > 0) && (allCategoryIds.length == 0) &&
		...
		if (allCategoryIds.length > 0) {
			entries = _getFilteredAssetEntriesByAllCategories(
				entries, allCategoryIds);
		}

		if (allTagNames.length > 0) {
			entries = _getFilteredAssetEntriesByAllTags(entries, allTagNames);
		}
	...
	private List<AssetEntry> _getFilteredAssetEntriesByAllCategories(
	...
	private List<AssetEntry> _getFilteredAssetEntriesByAllTags(
