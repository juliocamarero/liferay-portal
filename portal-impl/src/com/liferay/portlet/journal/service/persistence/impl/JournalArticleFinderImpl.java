...
public class JournalArticleFinderImpl
	...
	protected int doCountByG_C_S(
		...
		try {
			...
			if (ddmStructureKey.equals(
				...
				sql = StringUtil.replace(
					sql, "(structureKey = ?)",
					"((structureKey = ?) OR (structureKey = '') OR " +
						"(structureKey IS NULL))");
			}
	...
	protected int doCountByC_G_F_C_A_V_T_D_C_T_S_T_D_R(
		...
		try {
			...
			if (!isNullArray(ddmStructureKeys)) {
				sql = CustomSQLUtil.replaceKeywords(
					sql, "JournalArticle.structureKey", StringPool.LIKE, false,
					ddmStructureKeys);
			}

			if (!isNullArray(ddmTemplateKeys)) {
				sql = CustomSQLUtil.replaceKeywords(
					sql, "JournalArticle.templateKey", StringPool.LIKE, false,
					ddmTemplateKeys);
			}
	...
	protected List<JournalArticle> doFindByG_C_S(
		...
		try {
			...
			if (ddmStructureKey.equals(
				...
				sql = StringUtil.replace(
					sql, "(JournalArticle.structureKey = ?)",
					"((JournalArticle.structureKey = ?) OR " +
						"(JournalArticle.structureKey = '') OR" +
							"(JournalArticle.structureKey IS NULL))");
			}
	...
	private static final String _STRUCTURE_KEY_SQL =
		"(JournalArticle.structureKey LIKE ? [$AND_OR_NULL_CHECK$]) ";

	private static final String _TEMPLATE_KEY_SQL =
		"(JournalArticle.templateKey LIKE ? [$AND_OR_NULL_CHECK$]) ";
