	...
	<div class="entries-empty portlet-msg-info">
		<c:choose>
			<c:when test="<%= (fileEntryTypeId >= 0) %>">
				<c:choose>
					<c:when test="<%= total == 0 %>">
						<liferay-ui:message arguments="<%= dlFileEntryTypeName %>" key="there-are-no-documents-of-type-x" />
					</c:when>
					<c:otherwise>
						<liferay-ui:message arguments="<%= dlFileEntryTypeName %>" key="there-are-no-documents-of-type-x-on-this-page" />
					</c:otherwise>
				</c:choose>
			</c:when>
			<c:otherwise>
