...
List<String> selectedContent = (List<String>)request.getAttribute("render_controls.jsp-selectedContent");
	...
	<li class="handler-control">
		<c:choose>
			<c:when test="<%= controls[i] instanceof PortletDataHandlerBoolean %>">
				...
				if (Validator.isNotNull(className) && (manifestSummary.getModelCount(className) != -1)) {
					controlName += " (" + manifestSummary.getModelCount(className) + ")";
				}

				data.put("name", controlName);

				selectedContent.add(controlName);
