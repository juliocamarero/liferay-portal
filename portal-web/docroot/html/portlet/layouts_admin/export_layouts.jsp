...
<div id="<portlet:namespace />exportImportOptions">
	...
	<aui:form action='<%= exportPagesURL + "&etag=0&strip=0" %>' cssClass="lfr-export-dialog" method="post" name="fm1">
		...
		<div class="export-dialog-tree">
			...
			<%
			List<Portlet> siteExportablePortlets = LayoutExporter.getDataSiteLevelPortlets(company.getCompanyId());
			%>

			<c:if test="<%= !siteExportablePortlets.isEmpty() %>">
					...
					<ul class="lfr-tree unstyled">
						<li class="tree-item">
							...
							<ul class="hide" id="<portlet:namespace />selectContents">
								...
								siteExportablePortlets = ListUtil.sort(siteExportablePortlets, new PortletTitleComparator(application, locale));

								for (Portlet portlet : siteExportablePortlets) {
									String portletDataHandlerClass = portlet.getPortletDataHandlerClass();
