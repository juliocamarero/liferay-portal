...
String robots = ParamUtil.getString(request, "robots", RobotsUtil.getRobots(layoutSet));
%>
...
<aui:fieldset>
	<c:choose>
		<c:when test="<%= Validator.isNotNull(virtualHostName) %>">
			<aui:input cols="60" label="set-the-robots-txt-for-pages" name="robots" rows="15" type="textarea" value="<%= robots %>" />
		</c:when>
		<c:otherwise>
