...
<liferay-ui:icon-menu>
	<c:choose>
		...
		<c:otherwise>
			<c:choose>
				<c:when test="<%= !GroupLocalServiceUtil.hasUserGroup(user.getUserId(), group.getGroupId()) && membershipPolicy.isMembershipAllowed(group, user) %>">
					<c:choose>
						...
						<c:when test="<%= (group.getType() == GroupConstants.TYPE_SITE_RESTRICTED) && !MembershipRequestLocalServiceUtil.hasMembershipRequest(user.getUserId(), group.getGroupId(), MembershipRequestConstants.STATUS_PENDING) && membershipPolicy.isMembershipAllowed(group, user) %>">
