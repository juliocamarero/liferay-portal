	...
	<ul class="lfr-component sites">
		...
		%>
					...
					Liferay.Search.tokenList.add(
						{
							...
							fieldValues: '<%= GetterUtil.getString(curGroupId) %>',
							text: '<%= UnicodeFormatter.toString(groupName) %>'
						}
					);
				</aui:script>
			</c:if>
			...
			<li class="facet-value <%= groupId == curGroupId ? "current-term" : StringPool.BLANK %>">
				<a data-value="<%= curGroupId %>" href="javascript:;"><%= groupName %></a>
