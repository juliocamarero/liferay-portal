	...
	<div class="entries-empty portlet-msg-info">
		<c:choose>
			<c:when test="<%= (Validator.isNotNull(displayTerms.getStructureId())) %>">
				<c:choose>
					<c:when test="<%= total == 0 %>">
						<liferay-ui:message arguments="<%= name %>" key="there-are-no-documents-of-type-x" />
					</c:when>
					<c:otherwise>
						<liferay-ui:message arguments="<%= name %>" key="there-are-no-documents-of-type-x-on-this-page" />
					</c:otherwise>
				</c:choose>
			</c:when>
			<c:otherwise>
