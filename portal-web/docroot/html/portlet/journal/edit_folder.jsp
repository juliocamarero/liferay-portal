...
<aui:form action="<%= editFolderURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveFolder();" %>'>
	...
	<aui:fieldset>
		...
		<c:if test="<%= rootFolder || (folder != null) %>">
			...
			<%
			List<DDMStructure> ddmStructures = JournalFolderLocalServiceUtil.getFolderDDMStructures(PortalUtil.getCurrentAndAncestorSiteGroupIds(scopeGroupId), folderId, false);
