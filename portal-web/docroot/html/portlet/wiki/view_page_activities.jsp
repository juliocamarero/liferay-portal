						...
						<c:choose>
							...
							<c:when test="<%= socialActivity.getType() == SocialActivityConstants.TYPE_MOVE_ATTACHMENT_TO_TRASH %>">
								<liferay-ui:icon
									...
									label="<%= true %>"
									message='<%= LanguageUtil.format(pageContext, "activity-wiki-delete-the-attachment", new Object[] {socialActivityUser.getFullName(), attachmentTitle}) %>'
								/>
							</c:when>
						...
						<c:choose>
							...
							<c:when test="<%= socialActivity.getType() == WikiActivityKeys.UPDATE_PAGE %>">
								...
								<c:if test="<%= socialActivityWikiPage.getStatus() != WorkflowConstants.STATUS_APPROVED %>">
									<em class="activity-summary"><liferay-ui:message key="<%= WorkflowConstants.toLabel(socialActivityWikiPage.getStatus()) %>" /></em>
								</c:if>
							</c:when>
						</c:choose>
					</c:when>
