...
if (category != null) {
	...
	if (!layout.isTypeControlPanel()) {
		PortalUtil.addPortletBreadcrumbEntry(request, LanguageUtil.get(pageContext, "add-category[MessageBoard]"), currentURL);
	}
}
%>
...
<liferay-ui:header
	...
	title='<%= (category == null) ? "add-category[MessageBoard]" : LanguageUtil.format(pageContext, "edit-x", category.getName()) %>'
/>
	...
	<aui:fieldset>
		<c:if test="<%= parentCategoryId != MBCategoryConstants.DEFAULT_PARENT_CATEGORY_ID %>">
			...
			<c:if test="<%= category != null %>">
				<aui:field-wrapper label="parent-category[MessageBoard]">
