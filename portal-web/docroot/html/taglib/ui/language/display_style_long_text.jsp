...
for (int i = 0; i < locales.length; i++) {
	...
	String localeDisplayName = LocaleUtil.getLongDisplayName(locales[i], duplicateLanguages);
%>
	...
	<c:choose>
		...
		<c:otherwise>
			<aui:a cssClass="<%= cssClassName %>" href="<%= HttpUtil.addNamespacedParameter(formAction, namespace, name, currentLanguageId) %>" lang="<%= LocaleUtil.toW3cLanguageId(locales[i]) %>"><%= localeDisplayName %></aui:a>
		</c:otherwise>
	</c:choose>
