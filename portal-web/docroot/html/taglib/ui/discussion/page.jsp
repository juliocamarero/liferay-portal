			...
			<c:if test="<%= messagesCount > 1 %>">
				...
				<aui:layout>
					...
					for (i = 1; i <= messages.size(); i++) {
						...
						if ((!message.isApproved() && (message.getUserId() != user.getUserId()) && !permissionChecker.isGroupAdmin(scopeGroupId)) || !MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.VIEW)) {
							continue;
						}
									...
									<c:if test="<%= !hideControls %>">
										<ul class="lfr-discussion-actions">
											...
											<c:if test="<%= i > 0 %>">
												...
												<c:if test="<%= (themeDisplay.getUserId() == message.getUserId()) && MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, message.getMessageId(), userId, ActionKeys.UPDATE_DISCUSSION) %>">
												...
												<c:if test="<%= (themeDisplay.getUserId() == message.getUserId()) && MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, message.getMessageId(), userId, ActionKeys.DELETE_DISCUSSION) %>">
