	...
	<c:if test="<%= Validator.isNotNull(xsd) %>">
		<c:choose>
			<c:when test="<%= Validator.isNotNull(fieldName) %>">
				<%= DDMXSDUtil.getFieldHTMLByName(pageContext, classNameId, classPK, fieldName, fields, portletResponse.getNamespace(), fieldsNamespace, null, readOnly, requestedLocale) %>
			</c:when>
			<c:otherwise>
